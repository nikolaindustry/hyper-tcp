#!/usr/bin/env python3
"""
Second client for testing device-to-device communication in HyperTCP protocol
"""
...
class HyperTCPSecondClient:
    def __init__(self, host='localhost', port=8080, device_id=None):
        self.host = host
        self.port = port
        self.device_id = device_id or f"actuator_device_{int(time.time() * 1000) % 100000}"
        self.socket = None
    ...
    def handle_message(self, message):
        ...
        if msg_type == HYPER_TCP_CMD_RESPONSE:
            ...
            try:
                ...
                if command == 'control':
                    self.handle_control_command(payload_data)
    ...
    def handle_control_command(self, payload):
        """Handle control commands sent to this device"""
        device = payload.get('device')
        action = payload.get('action')
        value = payload.get('value')
        
        print(f"Control command received: {device} {action} {value}")
        ...
        # In a real implementation, you would know the sender's ID from the message
        # For now, we'll just print that we received the command
...
def main():
    """Main function for testing the HyperTCP second client"""
    # Create client with the same device ID as the first client to test multiple connections
    client = HyperTCPSecondClient('localhost', 8080, "sensor_device_001")
    ...
    try:
        ...
        welcome_payload = {
            "command": "welcome",
            "message": f"Hello from second connection of {client.device_id}"
        }
        ...
        while client.connected:
            ...
            if counter % 15 == 0:
                status_payload = {
                    ...
                    "device": "relay_1",
                    "state": "on" if counter % 30 < 15 else "off",
                    "timestamp": int(time.time() * 1000)
                }
                client.send_message("server", status_payload)
